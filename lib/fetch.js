...
exports.fetchTransactions = async function() {
  const accountsAndTransactions = await Promise.all(plaidAccountTokens.map(({ account, token }) => {
  ...
  return accountsAndTransactions.reduce((all, { account, transactions }) => {
    const filteredTransactions = filterTransactions(transactions);
    return all.concat(filteredTransactions).map(({ name, date, amount, category }) => ({
      account,
      name,
      date,
      amount: -amount,
